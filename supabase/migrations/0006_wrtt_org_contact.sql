-- ============================================================
-- WRTT – organization contact, and only organization contact.
--
-- Form 990 Part VII names officers with titles and hours per week and
-- carries no personal address, phone or email. That is not an omission
-- to work around; it is why this data is publishable at all. Measured
-- over 400 filings: the filer's own phone is present on all of them,
-- about half list a usable website, and 3 contained any email address
-- whatsoever - all belonging to the accounting firm that prepared them.
--
-- So a candidate is reachable through the organization they lead, which
-- is also the channel a stranger is supposed to use. Personal contact
-- details, if they are ever wanted, come from the publisher who knows
-- the person at the confirm step - not from a broker.
-- ============================================================

alter table wrtt.organization add column if not exists phone   text;
alter table wrtt.organization add column if not exists website text;

comment on column wrtt.organization.phone is
  'Filer phone from the 990 header. Organization, never personal.';
comment on column wrtt.organization.website is
  'Filer website from Form 990 Part VI, when given and not a placeholder.';
